...
if TYPE_CHECKING:
    from DQN.minigrid.core.minigrid import MultiGridEnv
...
class WorldObj:
    ...
    @staticmethod
    def decode(embedding: tuple) -> WorldObj:
...
class Agent(WorldObj):
    def __init__(
        ...
        #initial the state
        self.carrying: WorldObj | None = None
    ...
    def decode(type, color_index, direction, carrying, carrying_type, carrying_color):
        ...
        if obj_type == 'empty':
            ...
            v.carrying = WorldObj.decode(carrying_type, carrying_color, 0) if carrying else None
        else:
    ...
    def get_view_exts(self, view_size=None):
        ...
        botX = topX + view_size - 1
        botY = topY + view_size - 1
...
class DoorWID(Door):
    ...
    def toggle(self, env, agent, pos):
        # If the player has the right key to open the door
        if self.is_locked:
            if isinstance(agent.carrying, KeyWID) and agent.carrying.id == self.id:
                self.is_locked = False
                self.is_open = True
                return True
            return False
